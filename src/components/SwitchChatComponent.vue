...
<template>
  <div class="contacts-wrap">
    <div class="contact">
      <div class="contact-user" v-for="bot in botsArr"
          :key="bot"
          :class="{ 'active': bot === activeBot }"
          @click.prevent="changeToActiveBot(bot)">
            <div class="contact-user-foto">
...
export default {
  name: 'SwitchChatComponent',
  data() {
    return {
      activeBot: 'Jimmy',
      botsArr: ['Tonny', 'Linda', 'Jimmy', 'Walter', 'Teacher'],
    };
  },
  methods: {
    changeToActiveBot(item) {
      this.activeBot = item;
    },
  },
  watch: {
    activeBot(name) {
      eventBus.$emit('selectedBot', name);
    },
  },
};
</script>
